...
export default function ResetPasswordPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      toast.error(axios.isAxiosError(e) ? e.response?.data?.error : "Reset failed");
    ...
    return (
      <div className="w-full max-w-md animate-fade-in">
        <div className="bg-card border border-border rounded-2xl p-8 shadow-xl text-center">
          ...
          <Link href="/auth/forgot-password" className="text-primary text-sm hover:underline mt-4 block">Request a new link</Link>
        </div>
      </div>
    );
  }
          ...
          <>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              {["password", "confirmPassword"].map((field, i) => (
                <div key={field}>
                  <label className="text-sm font-medium mb-1.5 block">{i === 0 ? "New Password" : "Confirm Password"}</label>
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                    <input
                      {...register(field as "password" | "confirmPassword")}
                  ...
                  {errors[field as "password" | "confirmPassword"] && (
                    <p className="text-destructive text-xs mt-1">{errors[field as "password" | "confirmPassword"]?.message}</p>
                  )}
                </div>
              ))}
            ...
            <Link href="/auth/login" className="block text-center text-sm text-muted-foreground hover:text-foreground mt-6 transition-colors">
              Back to sign in
            </Link>
          </>
        )}
